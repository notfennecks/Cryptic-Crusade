...
func _ready():  #Runs this function when the scene is loaded.
	emit_signal("updated_health", health)  #Emits a signal to update the enemy's health.
...
func _physics_process(delta):  #Function that runs every frame to apply the enemy's physics.
	...
	for idx in range(get_slide_count()):  #Loops the amount of times the body has collided or changed direction in last call to move_and_slide().
		var collision = get_slide_collision(idx)  #Set info about last collision to variable named "collision".
		if collision.collider.name == "Player":  #If the last collision's collider name is "Player".
			return  #Return
...
func take_damage():  #Function for applying damage to enemy.
	...
	emit_signal("updated_health", health)  #Emit signal "updated_health" with the current health value.
